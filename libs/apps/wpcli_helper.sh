...
function wpcli_install_needed_extensions() {
    ...
    [[ ${install_type} == "docker"* ]] && wpcli_cmd="docker compose -f ${wp_site}/../docker-compose.yml run --rm wordpress-cli wp --allow-root > /dev/null 2>&1"
...
function wpcli_rocket_settings_export() {
    ...
    # Command
    ${wpcli_cmd} --allow-root --path="${wp_site}" rocket export
