...
function wpcli_check_version() {
    ...
    [[ ${install_type} == "docker"* ]] && wpcli_cmd="docker --log-level=error compose -f ${wp_site}/../docker-compose.yml run -T --rm wordpress-cli wp"
...
function wpcli_user_create_application_password() {
    ...
    [[ ${install_type} == "docker"* ]] && wpcli_cmd="docker --log-level=error compose -f ${wp_site}/../docker-compose.yml run -T -u 33 -e HOME=/tmp --rm wordpress-cli wp --no-color"
...
function wpcli_install_needed_extensions() {
    ...
    [[ ${install_type} == "docker"* ]] && wpcli_cmd="docker --log-level=error compose -f ${wp_site}/../docker-compose.yml run -T --rm wordpress-cli wp --allow-root"
...
function wpcli_plugin_verify() {
    ...
    ## -u 33 -e HOME=/tmp to avoid permission denied error: https://github.com/docker-library/wordpress/issues/417
    ## --no-color added to avoid unwanted wp-cli output
    [[ ${install_type} == "docker"* ]] && wpcli_cmd="docker --log-level=error compose -f ${wp_site}/../docker-compose.yml run -T -u 33 -e HOME=/tmp --rm wordpress-cli wp --no-color > /dev/null 2>&1"
