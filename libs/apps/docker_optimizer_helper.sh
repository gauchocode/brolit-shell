...
function docker_php_fpm_optimize() {
  ...
  log_subsection "Calculated Settings"
  display --indent 6 --text "Container: ${container_name}"
  display --indent 6 --text "PHP Version: ${php_version}"
  display --indent 6 --text "Dedicated RAM: ${ram_dedicated} MB"
  display --indent 6 --text "CPU Cores: ${cpus}"
  display --indent 6 --text "pm.max_children: ${pm_max_children}"
  display --indent 6 --text "pm.start_servers: ${pm_start_servers}"
  display --indent 6 --text "pm.min_spare_servers: ${pm_min_spare_servers}"
  display --indent 6 --text "pm.max_spare_servers: ${pm_max_spare_servers}"
...
EOF
  ...
  if [[ $? -eq 0 ]]; then
    ...
    log_event "info" "PHP-FPM optimized successfully" "false"
  else
...
function docker_nginx_optimize() {
  ...
  display --indent 6 --text "Container: ${container_name}"
  display --indent 6 --text "CPU Cores: ${cpus}"
  display --indent 6 --text "worker_processes: ${cpus}"
  display --indent 6 --text "worker_connections: ${worker_connections}"

  # Update worker_processes if not configured
  if ! grep -q "worker_processes" "${nginx_conf_file}"; then
    sed -i "1i worker_processes ${cpus};" "${nginx_conf_file}"
    log_event "info" "Added worker_processes to nginx.conf" "false"
  else
    sed -i "s/worker_processes.*/worker_processes ${cpus};/" "${nginx_conf_file}"
  ...
  if grep -q "fastcgi_buffers" "${nginx_conf_file}"; then
    sed -i "s/fastcgi_buffers.*/fastcgi_buffers 32 32k;/" "${nginx_conf_file}"
  fi

  if grep -q "fastcgi_buffer_size" "${nginx_conf_file}"; then
    sed -i "s/fastcgi_buffer_size.*/fastcgi_buffer_size 64k;/" "${nginx_conf_file}"
  fi

  display --indent 6 --text "- Updating configuration" --result "DONE" --color GREEN
  ...
  if [[ $? -eq 0 ]]; then
    ...
    log_event "info" "Nginx optimized successfully" "false"
  else
...
function docker_mysql_optimize() {
  ...
  # Display settings
  display --indent 6 --text "Container: ${container_name}"
  display --indent 6 --text "innodb_buffer_pool_size: ${innodb_buffer_pool_size}M"

  # Create MySQL configuration directory
  local mysql_conf_dir="${project_path}/.mysql_data/conf.d"
  ...
  if [[ ${exitstatus} -eq 0 ]]; then
    ...
    while [[ ${retries} -gt 0 ]]; do
      if docker compose -f "${compose_file}" exec -T mysql mysqladmin ping -h localhost 2>/dev/null | grep -q "mysqld is alive"; then
        ...
        log_event "info" "MySQL optimized successfully" "false"
        return 0
      fi
      sleep 2
      ((retries--))
