...
function tasks_handler() {
  ...
  restore)
    ...
    # Execute task
    execute_task_with_error_handling "restore-${STASK}" "subtasks_restore_handler" "${STASK}" "${DOMAIN}" "${FILE}" "${TVALUE}" "${NDOMAIN}" "${STORAGEMETHOD}" "${SOURCESERVER}" "${LOCALSTAGINGPATH}"
    exit_code=$?
    exit ${exit_code}
    ;;
