  ...
  if [ -n "${bk_path}" ]; then
    ...
    if [ ${bzip2_result} -eq 0 ]; then
      ...
      if [ "${dropbox_remove_result}" -eq 0 ]; then
        ...
        display --indent 2 --text "- Deleting old backup from Dropbox" --result "FAIL" --color YELLOW
...
make_files_backup() {
  ...
  if [[ "${lbzip2_result}" -eq 0 ]]; then
    ...
    #echo "ls -lah ${BAKWP}/${NOW}/${bk_file} | awk '{ print $5}'"

    if [[ ${BK_FL_SIZE} == *"ERROR"* ]]; then

      log_event "error" "${BK_FL_SIZE}" "false"
      ...
      log_event "success" "Backup ${BACKUPED_FL} created, final size: ${BK_FL_SIZE}" "false"
      ...
      log_event "success" "Backup uploaded" "false"
