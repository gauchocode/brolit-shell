...
function restore_backup_files() {
  ...
  if [[ -d ${destination_dir} ]]; then
    ...
    if [[ "${non_interactive}" == "true" ]]; then
      # Non-interactive mode: always continue (backup old project automatically)
      log_event "info" "Directory exists, backing up old project automatically (non-interactive)" "false"
    else
...
function restore_backup_database() {
  ...
  if [[ ${user_db_exists} -eq 0 ]]; then
    ...
    display --indent 8 --text "Check project configuration files after restore" --tcolor YELLOW
  fi
...
function restore_backup_project_database() {
  ...
  # Ask if the database is associated to an existing project
  ...
  if [[ $? -eq 0 ]]; then
    ...
    local project_dir="${PROJECTS_PATH}/${filename}"
    local project_install_type
    local project_type

    project_install_type="$(project_get_install_type "${project_dir}")"
    project_type="$(project_get_type "${project_dir}")"
    ...
    if [[ "${project_install_type}" == "docker-compose" ]]; then
      ...
      # Read variables from docker .env file
      local docker_env_file="${project_dir}/.env"
      local docker_project_name=""
      local docker_mysql_database=""
      if [[ -f "${docker_env_file}" ]]; then
      ...
      database_engine="$(project_get_configured_database_engine "${project_dir}" "${project_type}" "${project_install_type}")"

      # Build container name based on database engine
      local container_name=""
      ...
      # Non-Docker project: read credentials from project config
      database_engine="$(project_get_configured_database_engine "${project_dir}" "${project_type}" "${project_install_type}")"
      database_name="$(project_get_configured_database "${project_dir}" "${project_type}" "${project_install_type}")"
      database_user="$(project_get_configured_database_user "${project_dir}" "${project_type}" "${project_install_type}")"
      database_user_pass="$(project_get_configured_database_userpassw "${project_dir}" "${project_type}" "${project_install_type}")"

      if [[ -n "${database_engine}" && -n "${database_name}" && -n "${database_user}" && -n "${database_user_pass}" ]]; then

        restore_backup_database "${database_engine}" "" "${database_name}" "${database_user}" "${database_user_pass}" "${BROLIT_TMP_DIR}/${chosen_backup_to_restore}"
        return 0
    ...
    # New project flow: ask for stage and project name
    project_stage="$(project_ask_stage "prod")"
    [[ $? -eq 1 ]] && return 1
    ...
    # Detect engine from dump file, default to mysql
...
function docker_setup_configuration() {
  ...
  local project_domain_new="${3}"
  local mode="${4:-new}"  # 'new' for new installations, 'restore' for backup restorations

  # Check if docker and docker-compose are installed
  package_is_installed "docker-ce"
  [[ $? -eq 1 ]] && return 1

  # For restore mode, only update domain and skip all other .env modifications
  # This preserves the original database credentials and configuration from the backup
...
function restore_backup_from_local_cli() {
  ...
  # Restore site files (non-interactive)
  display --indent 6 --text "- Restoring project files"
  restore_backup_files "${domain}" "${project_install_type}" "true"
  ...
  if [[ -n "${find_result}" ]]; then
    ...
    if [[ -d "${PROJECTS_PATH}/${domain}" ]]; then
      ...
      if [[ -n "${db_engine}" && -n "${db_name}" && -n "${db_user}" ]]; then
        ...
        restore_backup_database "${db_engine}" "prod" "${db_name}" "${db_user}" "${db_pass}" "${db_backup}" "true"
      fi
    fi
  fi

  # Restore project files (non-interactive)
  display --indent 6 --text "- Restoring project files"
  local project_install_type
  project_install_type="$(project_get_install_type "${BROLIT_TMP_DIR}/${domain}")"
...
################################################################################
# Download backup from storage (CLI non-interactive, no restore)
#
# Downloads a backup file to a local directory without restoring it.
#
# Arguments:
#   ${1} = ${domain} - Project domain
#   ${2} = ${backup_date} - Optional: specific backup date (YYYY-MM-DD)
#   ${3} = ${output_dir} - Optional: output directory (default: /root/backups)
#   ${4} = ${backup_type} - Optional: backup type (site, database, project)
#
# Outputs:
#   0 if ok, 1 on error.
################################################################################

function download_backup_cli() {

  local domain="${1}"
  local backup_date="${2}"
  local output_dir="${3:-/root/backups}"
  local backup_type="${4:-site}"

  # Validate required params
  if [[ -z "${domain}" ]]; then
    log_event "error" "Domain is required for download" "true"
    display --indent 6 --text "- Domain parameter is required" --result "FAIL" --color RED
    return 1
  fi

  log_subsection "Download Backup (CLI)"
  ...
  # Build remote path
  local remote_path="${chosen_server}/projects-online/${backup_type}/${domain}"
  ...
  # Build full download path
  local full_backup_path="${remote_path}/${chosen_backup}"

  # Download backup (skip confirmation with "true" flag)
  display --indent 6 --text "- Downloading backup to ${output_dir}"
  storage_download_backup "${full_backup_path}" "${output_dir}" "true"
  ...
  # Send notification
  send_notification "${SERVER_NAME}" "Backup downloaded for ${domain} to ${output_dir}" "success"

  display --indent 6 --text "- Download completed" --result "DONE" --color GREEN
  display --indent 8 --text "Location: ${output_dir}/${chosen_backup}" --tcolor WHITE
  return 0

}

################################################################################
# List backups from storage (CLI non-interactive)
#
# Lists all available backups for a domain without downloading or restoring.
#
# Arguments:
#   ${1} = ${domain} - Project domain
#   ${2} = ${backup_type} - Optional: backup type (site, database, project)
#
# Outputs:
#   JSON with list of available backups
################################################################################

function list_backups_cli() {
  ...
  if [[ -z "${backup_list}" ]]; then
    echo "{\"domain\":\"${domain}\",\"backups\":[],\"count\":0}"
    return 0
  fi
  ...
  for backup in ${backup_list}; do
    ...
    local backup_date
    backup_date="$(echo "${backup}" | grep -Eo '[0-9]{4}-[0-9]{2}-[0-9]{2}')"
    ...
    json_array="${json_array}{\"file\":\"${backup}\",\"date\":\"${backup_date}\"}"
    ((count++))
  done

  json_array="${json_array}]"

  echo "{\"domain\":\"${domain}\",\"backup_type\":\"${backup_type}\",\"backups\":${json_array},\"count\":${count}}"
  return 0
