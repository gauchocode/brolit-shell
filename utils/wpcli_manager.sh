...
function wpcli_select_user() {
  ...
  if [[ ${install_type} == "docker"* ]]; then
    ...
    # Redirect stderr to /dev/null to avoid Docker Compose messages mixing with CSV output
    users_list=$(docker compose -f "${project_path}/docker-compose.yml" run -T --rm -u 33 -e HOME=/tmp wordpress-cli wp user list --fields=user_login,display_name,user_email,roles --format=csv 2>/dev/null)
    wpcli_result=$?

    if [[ ${wpcli_result} -ne 0 ]]; then
      log_event "error" "WP-CLI command failed with exit code ${wpcli_result}" "false"
    fi
  else
...
function wpcli_delete_posts_by_author() {
  ...
  # First, get the author ID from the username
  if [[ ${install_type} == "docker"* ]]; then
    project_path=$(dirname "${wp_site}")
    author_id=$(docker compose -f "${project_path}/docker-compose.yml" run -T --rm -u 33 -e HOME=/tmp wordpress-cli wp user get "${author_name}" --field=ID --quiet 2>/dev/null | tail -1)
  else
    author_id=$(wp user get "${author_name}" --path="${wp_site}" --field=ID --quiet 2>/dev/null)
  fi

  if [[ -z ${author_id} ]]; then
    display --indent 8 --text "User not found: ${author_name}" --tcolor RED
    log_event "error" "User not found: ${author_name}" "false"
    return 1
  fi

  log_event "debug" "Author ID for '${author_name}': ${author_id}" "false"

  # Count posts using author ID (more reliable than username)
  if [[ ${install_type} == "docker"* ]]; then
    log_event "debug" "Running: docker compose -f ${project_path}/docker-compose.yml run -T --rm -u 33 -e HOME=/tmp wordpress-cli wp post list --author=${author_id} --post_type=${post_type} --format=count" "false"
    post_count=$(docker compose -f "${project_path}/docker-compose.yml" run -T --rm -u 33 -e HOME=/tmp wordpress-cli wp post list --author="${author_id}" --post_type="${post_type}" --format=count --quiet 2>/dev/null | tail -1)
  else
  ...
  if whiptail --title "CONFIRM DELETION" --yesno "Found ${post_count} ${post_type} by author '${author_name}'.\n\nAre you sure you want to DELETE ALL these ${post_type}?\n\nThis action CANNOT be undone!" 14 70; then
    ...
    echo ""
    display --indent 6 --text "- Deleting posts in batches of ${batch_size}..." --tcolor YELLOW
    echo ""
    ...
    while [[ ${remaining} -gt 0 ]]; do
      ...
      if [[ ${exitstatus} -eq 0 && ${batch_deleted} -gt 0 ]]; then
        ...
        # Show progress
        display --indent 8 --text "Batch ${batch_num}: Deleted ${batch_deleted} posts (Total: ${deleted_total}/${post_count}, Remaining: ${remaining})" --tcolor GREEN
        log_event "info" "Batch ${batch_num}: Deleted ${batch_deleted} posts (${deleted_total}/${post_count})" "false"

        batch_num=$((batch_num + 1))

        # If no posts were deleted, we're done (prevents infinite loop)
        if [[ ${batch_deleted} -eq 0 ]]; then
          break
        fi
      else
        display --indent 8 --text "Batch ${batch_num}: FAILED" --tcolor RED
        log_event "error" "Batch ${batch_num} failed: ${wpcli_result}" "false"
        remaining=0  # Stop on error
        break
      fi
    ...
    echo ""
    if [[ ${deleted_total} -eq ${post_count} ]]; then
      display --indent 6 --text "- Successfully deleted all ${deleted_total} ${post_type}" --result "DONE" --color GREEN
      log_event "success" "Deleted ${deleted_total} ${post_type} by author: ${author_name}" "false"
    elif [[ ${deleted_total} -gt 0 ]]; then
      display --indent 6 --text "- Partially deleted ${deleted_total}/${post_count} ${post_type}" --result "WARNING" --color YELLOW
      log_event "warning" "Partially deleted ${deleted_total}/${post_count} ${post_type} by author: ${author_name}" "false"
    else
      display --indent 6 --text "- Failed to delete ${post_type}" --result "FAIL" --color RED
      log_event "error" "Failed to delete ${post_type} by author: ${author_name}" "false"
      return 1
    fi
  ...
  if whiptail --title "CONFIRM DELETION" --yesno "Found ${post_count} ${post_type} by author ID '${author_id}'.\n\nAre you sure you want to DELETE ALL these ${post_type}?\n\nThis action CANNOT be undone!" 14 70; then
    ...
    while [[ ${remaining} -gt 0 ]]; do
      ...
      if [[ ${exitstatus} -eq 0 && ${batch_deleted} -gt 0 ]]; then
        ...
        # If no posts were deleted in this batch, we're done
        if [[ ${batch_deleted} -eq 0 ]]; then
          break
        fi
      else
    ...
    if [[ ${deleted_total} -eq ${post_count} ]]; then
      display --indent 6 --text "- Successfully deleted all ${deleted_total} ${post_type}" --result "DONE" --color GREEN
      log_event "success" "Deleted ${deleted_total} ${post_type} by author ID: ${author_id}" "false"
    elif [[ ${deleted_total} -gt 0 ]]; then
      display --indent 6 --text "- Partially deleted ${deleted_total}/${post_count} ${post_type}" --result "WARNING" --color YELLOW
      log_event "warning" "Partially deleted ${deleted_total}/${post_count} ${post_type} by author ID: ${author_id}" "false"
    else
      display --indent 6 --text "- Failed to delete ${post_type}" --result "FAIL" --color RED
      log_event "error" "Failed to delete ${post_type} by author ID: ${author_id}" "false"
      return 1
    fi
...
function wpcli_delete_posts_by_date_range() {
  ...
  if whiptail --title "CONFIRM DELETION" --yesno "Found ${post_count} ${post_type} between ${start_date} and ${end_date}.\n\nAre you sure you want to DELETE ALL these ${post_type}?\n\nThis action CANNOT be undone!" 14 70; then
    ...
    if [[ ${deleted_total} -eq ${post_count} ]]; then
      display --indent 6 --text "- Successfully deleted all ${deleted_total} ${post_type}" --result "DONE" --color GREEN
      log_event "success" "Deleted ${deleted_total} ${post_type} from ${start_date} to ${end_date}" "false"
    elif [[ ${deleted_total} -gt 0 ]]; then
      display --indent 6 --text "- Partially deleted ${deleted_total}/${post_count} ${post_type}" --result "WARNING" --color YELLOW
      log_event "warning" "Partially deleted ${deleted_total}/${post_count} ${post_type} from ${start_date} to ${end_date}" "false"
    else
      display --indent 6 --text "- Failed to delete ${post_type}" --result "FAIL" --color RED
      log_event "error" "Failed to delete ${post_type} from ${start_date} to ${end_date}" "false"
      return 1
    fi
...
function wpcli_delete_posts_by_keyword() {
  ...
  if whiptail --title "CONFIRM DELETION" --yesno "Found ${post_count} ${post_type} containing '${keyword}'.\n\nAre you sure you want to DELETE ALL these ${post_type}?\n\nThis action CANNOT be undone!" 14 70; then
    ...
    if [[ ${deleted_total} -eq ${post_count} ]]; then
      display --indent 6 --text "- Successfully deleted all ${deleted_total} ${post_type}" --result "DONE" --color GREEN
      log_event "success" "Deleted ${deleted_total} ${post_type} with keyword: ${keyword}" "false"
    elif [[ ${deleted_total} -gt 0 ]]; then
      display --indent 6 --text "- Partially deleted ${deleted_total}/${post_count} ${post_type}" --result "WARNING" --color YELLOW
      log_event "warning" "Partially deleted ${deleted_total}/${post_count} ${post_type} with keyword: ${keyword}" "false"
    else
      display --indent 6 --text "- Failed to delete ${post_type}" --result "FAIL" --color RED
      log_event "error" "Failed to delete ${post_type} with keyword: ${keyword}" "false"
      return 1
    fi
...
function wpcli_delete_posts_by_status() {
  ...
  if whiptail --title "CONFIRM DELETION" --yesno "Found ${post_count} ${post_type} with status '${post_status}'.\n\nAre you sure you want to DELETE ALL these ${post_type}?\n\nThis action CANNOT be undone!" 14 70; then
    ...
    if [[ ${deleted_total} -eq ${post_count} ]]; then
      display --indent 6 --text "- Successfully deleted all ${deleted_total} ${post_type}" --result "DONE" --color GREEN
      log_event "success" "Deleted ${deleted_total} ${post_type} with status: ${post_status}" "false"
    elif [[ ${deleted_total} -gt 0 ]]; then
      display --indent 6 --text "- Partially deleted ${deleted_total}/${post_count} ${post_type}" --result "WARNING" --color YELLOW
      log_event "warning" "Partially deleted ${deleted_total}/${post_count} ${post_type} with status: ${post_status}" "false"
    else
      display --indent 6 --text "- Failed to delete ${post_type}" --result "FAIL" --color RED
      log_event "error" "Failed to delete ${post_type} with status: ${post_status}" "false"
      return 1
    fi
